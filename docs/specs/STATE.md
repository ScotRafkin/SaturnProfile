...
SPEC_04 accepted by the author at v0.6 on 21 September 2026 (now v0.12). Steps 0 to 5 proceed in order, each after the review of the one before; the coding agent's pre-execution review of v0.5 is ruled on in SPEC_04 section 10, its record is `reports/REPORT_04_preexecution.md`, and the rulings on the step reports are in sections 11 (Step 0), 12 (Step 1), 13 (Step 2) and 14 (Step 3, with the v0.13 addendum accepting the refreshed report). Current: SPEC_00 v0.20, SPEC_01 v0.28, SPEC_02 v0.11, SPEC_03 v0.14, SPEC_04 v0.13. The manuscript draft of record for the equation labels is `CASSPIAN_AtmosphericModel_Draft9_2.docx`, outside the repository. Next specification after SPEC_04: the combination (A35, the posterior wind, the reference-surface constant), then SPEC_05, the end-to-end tests.
...
| 2 | `lib.mesh`, the columns, `z_lv` | accepted (`562c769`; 13 of 13 under REVIEW_04_step2 and SPEC_04 v0.11, the §13 rulings applied; the column at `phi_c` reproduces every stated `z_lv` and `r - r0` to 0.050 m, an extended mesh and one built from the start give columns agreeing exactly, and the cylinder-extended wind of decision P converges in four passes with its reference-level identity exactly zero at all 361 latitudes; six findings, two of them defects this step's checks caught, decision P per hemisphere and `edge_order = 2`, three the specification's numbers restated by rulings 1 to 3, and the sixth resolved by ruling 3 from 0.054 to 0.006 against the same bound with nothing loosened; full regression passes, every accepted suite at its reference count; nothing registered and no product changed, so no sweep) | `reports/REPORT_04_step2.md`, `reports/REVIEW_04_step2.md` | 2026-09-22 |
| 3 | `lib.kernel` | accepted (`bb75d16`; 9 of 9 under REVIEW_04_step3 and SPEC_04 v0.12, the §14 rulings applied; under the closure wind `S/g` is the closed form to 4.163e-17 with `du/dln p` exactly zero and no difference taken, `I` exactly zero at the gauge node of every column, the composition term exact for identical columns; under the cylinder wind the line integrals give `d ln N` 4.746e-04 against 2e-3 and a shift of 130.3 m2/s2 against 300, at decision Q's truncation floor; five findings, one a real defect in `lib.kernel` (`np.gradient` cannot return an exact zero, now in difference form), two the specification's and restated by rulings 1 and 5, two the acceptance script's, and the first filing's claim that decision L and Eq. A15 are in tension at order one withdrawn; full regression passes, every accepted suite at its reference count; nothing registered and no product changed, so no sweep; the refreshed report verified and Step 3 accepted by the SPEC_04 v0.13 addendum to section 14, which states the per-level cylinder line integrals as a size class with only their bounds checked, the floor for Step 4 the two kernels' values together, 130 to 224 m2/s2 and 4.7e-4 to 1.1e-3) | `reports/REPORT_04_step3.md`, `reports/REVIEW_04_step3.md` | 2026-09-23 |
