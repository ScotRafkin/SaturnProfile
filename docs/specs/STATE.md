...
| 1 | `lib.geoid` `through_anchor`, `lib.windfield`, the anchors' geopotential under the run's wind (`produce` with the wind along the column) | reported (12 of 14; deliverables 1 and 3 reproduce every stated value, the closure production bit-identical to the registered product; four findings, finding 2 on the cylinder-extended wind, whose construction the specification does not state and whose stated column values and top geopotential this one misses by a single factor near 1.30, and finding 4 on `step04_0` check 0, vacuous on a clean tree; twenty of the twenty-two regression suites at their reference counts; nothing registered, the closure product to be rebuilt at the sweep) | `reports/REPORT_04_step1.md` | 2026-09-21 |
