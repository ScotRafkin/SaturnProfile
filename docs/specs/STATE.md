...
| 4 | `forward/production.py`, `casspian-forward`, the closure run, F5 and F6 | reviewed, accepted (REVIEW_03_step4; closure reproduced independently to 6e-16; check 5 failed on a wording defect of the specification, bins restated in pressure at SPEC_03 v0.13 and passing on the measured numbers); before the acceptance commit: the v0.13 bins and `date` in the closure namelist, acceptance rerun; then the acceptance commit, the closure rerun on the clean tree with F5 and F6 attached, the record commit | `reports/REPORT_03_step4.md`, `reports/REVIEW_03_step4.md` | 2026-09-16 |
